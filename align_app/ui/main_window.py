# [FULL FILE — paste replaces your current main_window.py]
...
from PyQt5.QtWidgets import (
    QMainWindow, QSplitter, QWidget, QVBoxLayout, QToolBar, QAction, QFileDialog,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Left: compact sidebar with 4 sections (Base, Source, Align Out, Crops Out)
    ...
    def _build_align_toolbar(self):
        ...
        # Group 1: Navigation & Save (absorb QAction 'checked' bool)
        ...
        # Group 2: Move buttons — FIXED: bind dx,dy and ignore 'checked'
        for label, dx, dy in [("←", -1, 0), ("→", +1, 0), ("↑", 0, -1), ("↓", 0, +1)]:
            add(QAction(label, self,
                        triggered=(lambda dx=dx, dy=dy:
                                   (lambda checked=False: self.canvas.move_dxdy(dx*self.canvas.step,
                                                                                dy*self.canvas.step)))()))
        self.toolbar_align.addSeparator()

        # Group 3: Step / Rotate / Zoom (absorb 'checked')
        add(QAction("Step−", self, triggered=lambda checked=False:
                    setattr(self.canvas, "step", max(0.5, self.canvas.step - 1.0))))
        add(QAction("Step+", self, triggered=lambda checked=False:
                    setattr(self.canvas, "step", min(50.0, self.canvas.step + 1.0))))
        self.toolbar_align.addSeparator()
        add(QAction("Rot−", self, triggered=lambda checked=False: self.canvas.rotate_deg(-self.canvas.rot_step)))
        add(QAction("Rot+", self, triggered=lambda checked=False: self.canvas.rotate_deg(+self.canvas.rot_step)))
        self.toolbar_align.addSeparator()
        add(QAction("Zoom−", self, triggered=lambda checked=False: self.canvas.zoom_factor(1.0 - self.canvas.scale_step)))
        add(QAction("Zoom+", self, triggered=lambda checked=False: self.canvas.zoom_factor(1.0 + self.canvas.scale_step)))
        add(QAction("µZoom−", self, triggered=lambda checked=False: self.canvas.zoom_factor(1.0 - self.canvas.micro_scale_step)))
        add(QAction("µZoom+", self, triggered=lambda checked=False: self.canvas.zoom_factor(1.0 + self.canvas.micro_scale_step)))
        self.toolbar_align.addSeparator()

        # Group 4: View toggles
        add(QAction("Reset", self, triggered=lambda checked=False: self.canvas.reset_current()))
        add(QAction("Overlay", self, triggered=lambda checked=False: self._toggle_overlay()))
        add(QAction("Alpha−", self, triggered=lambda checked=False:
        ...
        add(QAction("Grid−", self, triggered=lambda checked=False:
        ...
        add(QAction("Outline", self, triggered=lambda checked=False: self._toggle_outline()))
        self.toolbar_align.addSeparator()

        # Group 5: Crop
        add(QAction("Crop", self, triggered=lambda checked=False: self.canvas.start_crop_mode()))

        # Repaint canvas after toolbar actions
        self.toolbar_align.actionTriggered.connect(lambda _a: self.canvas.update())
    ...
    def _start_status_timer(self):
        ...
        self._timer.start(150)  # update ~6-7 fps

    def _refresh_status(self):
        # Move step is in preview px. Rot in deg. Zoom steps in %.
        ...
        txt = (f"Move step: {step:.1f}px   Rot step: {rstep:.2f}°   "
    ...
    def _pick_base_image(self):
        fn, _ = QFileDialog.getOpenFileName(self, "Choose Base Image", str(Path.home()),
    ...
    def _rebuild_sidebar(self):
        ...
        # If we just set a source and have files, show the first image
    ...
    def _add_dir_tree(self, parent_item: QTreeWidgetItem, dir_path: Path):
        """Recursively add directory contents to the sidebar."""
        try:
    ...
    def _sidebar_double_clicked(self, item: QTreeWidgetItem, _col: int):
        ...
        if p.is_file() and p.suffix.lower() in SUPPORTED_LOWER:
            # If it's under the Source tree, set current image to this file
            if self.canvas.files:
                try:
                    idx = self.canvas.files.index(p)
                except ValueError:
                    # not in flat list yet: ensure src_dir is its ancestor; keep list recursive (already is)
                    idx = None
                    # If file is in source dir tree, insert it if missing
                    # (Normally covered by rglob, so this path rarely happens.)
                if idx is not None:
                    self.canvas.idx = idx
                    self.canvas.update()
        elif p.is_dir():
            # If this is a directory node (e.g., under Source), set as src dir
